...
import { useRoute } from '#app'; // 使用 Nuxt 提供的工具
...
useHead({
  title: '客房旅宿-享樂酒店｜Enjoyment Luxury Hotel',
});
definePageMeta({
  name: 'rooms',
  layout: 'front', // 與文件名一致
...
{
  // 使用 import.meta.glob 動態導入 assets 資料夾內的圖片
...
const roomImages = computed(() =>
{
  ...
  const result = rooms.reduce((acc, roomId) =>
  {
    ...
    {
      obj[num] = {
        desktop: importImage(`room-${roomId}-${num}.png`), // 桌面版圖片
        mobile: importImage(`room-${roomId}-sm-${num}.png`) // 手機版圖片
      };
      return obj;
    }, {});
  ...
  <main>
    ...
    <section class="py-10 py-md-30 bg-primary-10">
      <div class="container mb-md-12">
        ...
        <!-- 顯示載入中 -->
        <div v-if="roomsPending" class="text-center">載入中...</div>

        <!-- 顯示錯誤訊息 -->
        <div v-if="roomsError" class="text-center text-danger">
          無法載入房型資料：{{ roomsError.message }}
        </div>

        <!-- 動態渲染房型資訊 -->
        <ul v-if="!roomsPending && !roomsError" class="d-flex flex-column gap-6 gap-md-12 list-unstyled">
          <li v-for="room in roomsData" :key="room._id" class="card flex-lg-row border-0 rounded-3xl overflow-hidden">
            <div class="row">
                ...
                <swiper :modules="modules" :slides-per-view="1" navigation :pagination="{ clickable: true }" :autoplay="{
                  ...
                  <swiper-slide v-for="(image, index) in room.imageUrlList" :key="index">
                    <picture>
                      ...
                      <img class="w-100 object-fit-cover" :src="image" :alt="room.name">
                    </picture>
                  </swiper-slide>
                </swiper>
              </div>
              <div class="col-12 col-lg-5">
                <div class="card-body pe-md-10 py-md-10">
                  <h3 class="card-title fs-2 fw-bold text-neutral-100">{{ room.name }}</h3>
                  <p class="card-text mb-6 mb-md-10 fs-8 fs-md-7 fw-medium text-neutral-80">{{ room.description }}</p>
                  <ul class="d-flex gap-4 mb-6 mb-md-10 list-unstyled">
                    <li class="card-info px-4 py-5 border border-primary-40 rounded-3">
                      ...
                      <p class="mb-0 fw-bold text-neutral-80">{{ room.areaInfo }}</p>
                    </li>
                    <li class="card-info px-4 py-5 border border-primary-40 rounded-3">
                      <Icon class="mb-2 fs-5 text-primary-100" icon="material-symbols:king-bed" />
                      <p class="mb-0 fw-bold text-neutral-80">{{ room.bedInfo }}</p>
                    </li>
                    <li class="card-info px-4 py-5 border border-primary-40 rounded-3">
                      <Icon class="mb-2 fs-5 text-primary-100" icon="ic:baseline-person" />
                      <p class="mb-0 fw-bold text-neutral-80">{{ room.maxPeople }} 人</p>
                    </li>
                  </ul>
                  <div class="deco-line w-100 mb-6 mb-md-10"></div>
                  <div class="d-flex justify-content-between align-items-center fs-7 fs-md-5 text-primary-100">
                    ...
                    <NuxtLink :to="{ name: 'room-detail', params: { roomId: room._id } }" class="icon-link icon-link-hover text-primary-100">
