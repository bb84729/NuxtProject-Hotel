   ...
   <main class="overflow-hidden">
    ...
    <div class="container position-relative">
      <div class="row">
        ...
        <div class="col-12 col-md-10 d-flex flex-column gap-10">
          <!-- 顯示載入中狀態 -->
          <div v-if="newsPending" class="text-center">載入中...</div>
          <!-- 顯示錯誤訊息 -->
          <div v-if="newsError" class="text-center text-danger">
            無法載入新聞：{{ error.message }}
          </div>
    ...
    <div class="container">
      ...
      <div class="row flex-nowrap overflow-x-auto">
        <!-- 顯示載入中狀態 -->
        <div v-if="culinaryPending" class="text-center">載入中...</div>
        <!-- 顯示錯誤訊息 -->
        <div v-if="culinaryError" class="text-center text-danger">
          無法載入資料：{{ error.message }}
        </div>

        <!-- 動態渲染美食資料 -->
        <div
          v-for="item in culinaryData"
          :key="item._id"
...
// 獲取新聞資料
const { data: newsDataRaw, pending: newsPending, error: newsError } = await useFetch('https://freyja-rfio.onrender.com/api/v1/home/news/');
const newsData = ref([]);
if (newsDataRaw) {
...
// 獲取美食資料
const { data: culinaryDataRaw, pending: culinaryPending, error: culinaryError } = await useFetch('https://freyja-rfio.onrender.com/api/v1/home/culinary/');
...
console.log('Culinary Data:', culinaryData.value);
// 配置 Swiper 模組
